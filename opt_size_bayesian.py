...
gridpoints = np.genfromtxt('./sourcemeshgridpoints/gridpointsfinal.csv',
                           delimiter=',').tolist()
...
def average_distance(x):
    ...
    print(opt_degree_u, opt_degree_v, opt_ctrlpts_size_u, opt_ctrlpts_size_v, ', and deviation: ', averagedeviation, maxdeviation, time.strftime("%H:%M:%S"))
...
opt_degree_u = int(round(res.x[0]))
opt_degree_v = int(round(res.x[1]))
opt_ctrlpts_size_u = int(round(res.x[2]))
opt_ctrlpts_size_v = int(round(res.x[3]))

surf = fitting.approximate_surface(gridpoints, 97, 161, degree_u=opt_degree_u, degree_v=opt_degree_v,
                                   ctrlpts_size_u=opt_ctrlpts_size_u, ctrlpts_size_v=opt_ctrlpts_size_v)
surf.delta = 0.01
